...
The JAR in `libs/` is the plugin release artifact; keep it unchanged when distributing.
...
Manual installation bypasses the managed safeguards (UI confirmation, preflight, staging, backup/restore); the JAR still goes through startup validation. See [Plugin Management](../use/plugin-management) for the full manual procedure and the update/uninstall rules.
