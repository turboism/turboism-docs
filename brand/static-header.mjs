import { persistLanguagePreference, readLanguagePreference } from './language-preference.mjs';
...
export function renderStaticHeader(active = 'sdk', locale = 'en') {
 ...
 const language=`<div class="tb-language"><div class="tb-locale" role="group" aria-label="Interface language">${['en', 'zh', 'ja', 'ko'].map(value=>`<button type="button" data-tb-lang="${value}" lang="${value==='zh'?'zh-CN':value}" aria-pressed="${value===locale}">${({en:'EN',zh:'中文',ja:'日本語',ko:'한국어'})[value]}</button>`).join('')}</div></div>`;
 const socials=`<div class="tb-socials">${navigation.socials.map(item=>`<a class="tb-icon" href="${escape(item.href)}" target="_blank" rel="noopener noreferrer" aria-label="Turboism ${item.name}" title="${item.name}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="${item.path}"/></svg></a>${item.name==='Discord'?qqMarkup(locale):''}`).join('')}</div>`;
